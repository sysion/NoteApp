...
function getNoteFromStorage(){
	if (isLocalStorage) {
     	return JSON.parse(localStorage.getItem("notes")) || [];  //results in an array of note objects.
    }
}
...
function AddNote(newText){
	...
	if (newText){
		...
		//replace newline(\n or \r\n) with </br> to display properly in div
	...
	deleteButton.addEventListener('click', function(e){
		...
		//var delNoteId = e.target.closest('.notecontainer').id;
		//var delNoteId = e.target.parentNode.parentNode.parentNode.parentNode.id;
		...
		// update notes array and save in localstorage
		notes.splice(index, 1);
		deleteNoteFromStorage(notes);

		// remove deleted note from page
	...
	noteArea.addEventListener('change', function(e){
		...
		// below format works using variable
		var savedNote = {};
		...
		var curNote = [];
		if (newText){
			//if (notes.length > 0){ //this is true since newText is true

			// check if noteId is already saved
			for (var i = 0; i < notes.length; i++){
				console.log("notes[i] => "+Object.keys(notes[i])[0]);
				//console.log("notes[i] => "+Object.entries(note)[0]);
			...
			if (curNote.length > 0){
				...
				//console.log("curNote => "+Object.fromEntries(curNote[0])[0]);
				// get index in notes array and update current value
				index = notes.findIndex(function(obj){
					//console.log("obj => "+Object.keys(obj)[0]);
				...
				// update newNote text in notes array
				notes[index] = savedNote;
			}

		}
		else {
			// add newNote text to notes array
